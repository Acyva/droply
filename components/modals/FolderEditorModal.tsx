...
export default function FolderEditorModal({
  ...
  const handleSave = async () => {
    if (!name.trim()) return;
    setLoading(true);
    await updateFolder(folderId, { name: name.trim(), icon, color });
    setLoading(false);
    onSave?.();
    onClose();
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm" onClick={e => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="bg-white dark:bg-stone-900 rounded-2xl shadow-2xl w-full max-w-md border border-stone-200 dark:border-stone-700 overflow-hidden">
        <div className="flex items-center justify-between px-5 py-4 border-b border-stone-100 dark:border-stone-800">
          <h2 className="text-sm font-semibold text-stone-900 dark:text-stone-50">Edit Folder</h2>
        ...
        <div className="px-5 py-4 space-y-4 max-h-[calc(100vh-200px)] overflow-y-auto">
          <div>
            ...
            <input
              ...
              className="w-full text-sm px-3 py-2 bg-stone-50 dark:bg-stone-800 rounded-lg border border-stone-200 dark:border-stone-700 outline-none focus:border-stone-400 dark:focus:border-stone-500"
